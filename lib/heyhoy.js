...
function heyhoy(code, verbose = false)
{
        ...
        function action()
        {
                Log.v(logIndenter(1), 'Enter Action');

                /** @type {string} */
                const prev_lex = lexemes[current_lex_i - 1][1];
                ...
                Log.v(logIndenter(-1), 'Exit Action');
        }
        ...
        function expression()
        {
                ...
                // left_num is the part before add sign
                // right_num is the part after add sign
                ...
                const left_num = STACK.pop();

                if (current_tok === TOK_ADD_SIGN)
                {
                        // If there's number on right
                ...
                {
                        // If there's no addition, return the left number alone
                        result = left_num;
                }
        ...
        function factor()
        {
                ...
                {
                        // If there's no multiplication, return the left number alone
                        result = left_num;
                }
